...
(function(){
  ...
  if(!document.body){setTimeout(arguments.callee,60);return}

  var stored=localStorage.getItem('__pokerAgentActive__');
  window.__pokerAgentActive__=(stored==='true');
...
<div class="panel">
  ...
  toggle.addEventListener('click',function(){
    ...
    localStorage.setItem('__pokerAgentActive__',String(window.__pokerAgentActive__));
    update();
  });
  update();
})();
...
class AgentOverlay:
    ...
    async def setup(self, url: str) -> None:
        ...
        await page.context.add_init_script(_INIT_JS)
